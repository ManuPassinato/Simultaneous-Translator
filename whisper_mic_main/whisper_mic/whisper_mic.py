...
class WhisperMic:
    def __init__(self,model="base",device=("cuda" if torch.cuda.is_available() else "cpu"),english=False,verbose=False,energy=300,pause=2.0,dynamic_energy=False,save_file=False):
    ...
    def transcribe(self,data=None,realtime=False):
        ...
        result = self.audio_model.transcribe(audio_data,language='Portuguese')
        print("Voce falou: " + result["text"])
        result = self.audio_model.transcribe(audio_data,language='Portuguese', task='translate')
